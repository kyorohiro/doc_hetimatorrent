...
実際には、Seeder化したPeerへはunchokeする必要はないので、
基本、アップロード側でChokeが行われるので、自分が欲しいデータを持っているPeerに対しては、Intersetメッセージを送信する。Unchokeメッセーシを受け取ったら、積極的にダウンロードを繰り返すだけでもそこそこのパフォーマンスが出ます。
 

このように、良い感じにunchokeされる数は調整されます。しかし、seeder化したpeerが増えてくると、Unchokeしてくれるpeer数が爆発します。
 

どのSeederを選ぶのがより良い選択なのかするこてで、効率的にダウンロードすることができるようになります。また、利用する帯域をコントロールすることで、Torrent Clientが回線すべてを占有しないようにできます。
